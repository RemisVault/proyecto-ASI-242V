...
if (!preg_match('/^[A-Z0-9\s_-]+$/', $nombre_red)) {
    ...
    echo "<p><a href='delete_redes.html'><button type='button'>Volver</button></a></p>";
    echo "</center>";
    oci_close($conn);
    exit;
}
...
if (!$result) {
    ...
    echo "<p style='color:red;'>No se pudo completar la operation debido a un conflicto de integridad en el sistema.</p>";
    echo "<p><a href='delete_redes.html'><button type='button'>Volver</button></a></p>";
    echo "</center>";
    oci_free_statement($stmt);
    oci_close($conn);
...
echo "<br><p><a href='delete_redes.html'><button type='button'>Volver</button></a></p>";
echo "</center>";
